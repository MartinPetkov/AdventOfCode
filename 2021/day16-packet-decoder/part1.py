...
class Packet:
  ...
  def __init__(self, version, type_id, type_name, value):
  ...
  def __repr__(self):
    return f'<Packet v{self.version}; type={self.type_name}; value={self.value}>'
...
def parse_packet(bits):
  ...
  return Packet(version, type_id, type_name, value)
...
def solve(data):
  ...
  for line in lines:
    ...
    print(f'{line} = {packet.version_sum()}\t{packet}')
